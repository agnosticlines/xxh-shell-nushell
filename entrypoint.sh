...
if [[ ! -f .entrypoint-check-done ]]; then
  check_result=`$nu_bin -version 2>&1`
...
cd $HOME
# TODO: get nu to load plugins, just use rc.elv? Except for file execute which is a noninteractive context :(
$nu_bin "${EXECUTE_COMMAND[@]}" $EXECUTE_FILE
